...
| # | 英文原串 (English String) | 简体中文翻译 (Simplified Chinese) |
| :--- | :--- | :--- |
| 1. | This application is in development currently and should NOT be used for live surveys at the moment. | 本应用目前处于开发阶段，暂时不应被用于正式问卷。 |
| 2. | Surveys for health care | 医疗保健问卷 |
| 3. | Go to Surveys | 前往问卷 |
| 4. | Drag-and-drop groups and questions, apply DaisyUI themes per survey, and control access with roles. | 拖放组和问题，为每个问卷应用 DaisyUI 主题，并使用角色控制访问。 |
| 5. | Create a survey | 创建问卷 |
| 6. | Invite your team, scope visibility by organisation and survey membership, and manage participation securely. | 邀请您的团队，按组织和问卷成员资格划分可见性范围，并安全地管理参与。 |
| 7. | Manage surveys | 管理问卷 |
| 8. | Monitor dashboards, export CSV, and audit changes. Everything runs on a secure SSR stack. | 监控仪表板，导出 CSV，并审计更改。一切都在安全的 SSR 堆栈上运行。 |
| 9. | View dashboards | 查看仪表板 |
| 10. | Organisation created. You are now an organisation admin and can host surveys and build a team. | 组织机构已创建。您现在是组织机构管理员，可以托管问卷并建立团队。 |
| 11. | Question Group Builder | 问题组构建器 |
| 12. | Survey Dashboard | 问卷仪表板 |
| 13. | All Questions | 所有问题 |
| 14. | Groups | 组 |
| 15. | Create and manage groups, then add questions inside each group. | 创建和管理组，然后将问题添加到每个组内。 |
| 16. | Open a group | 打开一个组 |
| 17. | Import questions | 导入问题 |
| 18. | Overwrite survey questions? | 覆盖问卷问题吗？ |
| 19. | Groups and questions are assigned unique IDs as you edit. Use these IDs to reason about branching later. | 在您编辑时，组和问题会被分配唯一的 ID。使用这些 ID 进行后续的分支逻辑推理。 |
| 20. | Start typing (or use the sample markdown) to see the survey structure. | 开始输入（或使用示例 Markdown）以查看问卷结构。 |
| 21. | `# Group Title`, followed by a line for the group description | `# 组标题`，后跟一行用于组描述 |
| 22. | `## Question Title`, followed by a line for the question description | `## 问题标题`，后跟一行用于问题描述 |
| 23. | Optional: Required questions | 可选：必填问题 |
| 24. | Mark a question as required by adding an asterisk `*` | 通过添加星号 `*` 将问题标记为必填 |
| 25. | Required questions must be answered before the form can be submitted | 必填问题必须在提交表单之前回答 |
| 26. | Place the asterisk immediately after the question title text | 将星号紧跟在问题标题文本之后 |
| 27. | Works with all question types | 适用于所有问题类型 |
| 28. | Optional: Collections with REPEAT | 可选：带 **REPEAT** 的集合 |
| 29. | To mark a group as repeatable, add a line with **REPEAT** (or **REPEAT-5** to cap it) immediately above the group heading. | 要将组标记为可重复，请在组标题上方紧跟一行添加 **REPEAT**（或 **REPEAT-5** 进行限制）。 |
| 30. | To define a child collection nested under a repeatable parent, indent with `>` and add **REPEAT** above the child group: | 要定义嵌套在可重复父级下的子集合，请使用 `>` 缩进并在子组上方添加 **REPEAT**： |
| 31. | Use `>` before REPEAT and the group heading to indicate one level of nesting. | 在 REPEAT 和组标题前使用 `>` 表示一个级别的嵌套。 |
| 32. | Groups without REPEAT are normal, non-collection groups. | 没有 REPEAT 的组是正常的非集合组。 |
| 33. | Optional: Questions with conditional branching | 可选：带条件分支的问题 |
| 34. | Branching rules must start with `? when` and reference a question or group ID in curly braces. | 分支规则必须以 `? when` 开头，并在大括号中引用问题或组 ID。 |
| 35. | Operators: `equals`, `not_equals`, `contains`, `not_contains`, `greater_than`, `less_than`. | 运算符：`equals`（等于）、`not_equals`（不等于）、`contains`（包含）、`not_contains`（不包含）、`greater_than`（大于）、`less_than`（小于）。 |
...
| 72. | Remove this group from its repeat? | 将此组从其重复中移除？ |
...
| 8. | Live structure preview | 实时结构预览 |
...
| 50. | You don't have an organisation to manage yet. | 您还没有可管理的组织机构。 |
...
| 85. | What's this? | 这是什么？ |
| 86. | Visibility | 可见性 |
| 87. | Authenticated: participants must log in before accessing. Public: anyone with the link. Unlisted: secret link, no directory listing. | 已认证：参与者必须先登录才能访问。公开：任何拥有链接的人。未列出：秘密链接，不在目录中列出。 |
