...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not complete_strings_file.exists():
            self.stdout.write(self.style.ERROR(f"ERROR: {complete_strings_file} not found"))
            return

        master_strings = self.parse_markdown_file(complete_strings_file)
        self.stdout.write(f"Found {len(master_strings)} numbered strings in master list\n")
        ...
        for number, item in master_strings.items():
            ...
            # Debug first few
            if number <= 3:
                self.stdout.write(f"DEBUG #{number}: Master='{item['english'][:40]}' Cleaned='{master_cleaned[:40]}'")
            ...
            for entry in english_entries:
                ...
                if master_cleaned == entry_cleaned:
                    ...
                    found = True
                    if number <= 3:
                        self.stdout.write(f"  ✓ Matched to msgid='{entry['msgid'][:40]}'")
                    break
        ...
        self.stdout.write(f"Matched {len(english_lookup)} strings between .po and master list")
        if unmatched_numbers and len(unmatched_numbers) <= 10:
            self.stdout.write(self.style.WARNING(f"Could not match {len(unmatched_numbers)} master list entries:"))
        ...
        for md_file, locale_code in self.LANGUAGE_MAP.items():
            ...
            if not md_path.exists():
                self.stdout.write(self.style.WARNING(f"⚠ Skipping {locale_code}: {md_path} not found"))
                continue

            if not po_path.exists():
                self.stdout.write(self.style.WARNING(f"⚠ Skipping {locale_code}: {po_path} not found"))
                continue
            ...
            updated_count, not_found = self.update_po_entries(po_entries, translations, english_lookup)

            if not_found:
                self.stdout.write(self.style.WARNING(f"  ⚠ Could not match {len(not_found)} translations"))

            if not dry_run:
                # Write back to .po file
                self.write_po_file(po_path, po_entries)
                self.stdout.write(self.style.SUCCESS(f"✓ Updated {updated_count} translations in {po_path.name}"))
            else:
                self.stdout.write(f"[DRY RUN] Would update {updated_count} translations in {po_path.name}")

            total_updated += updated_count

        self.stdout.write(f"\n{'='*60}")
        if not dry_run:
            self.stdout.write(self.style.SUCCESS(f"✓ Total: {total_updated} translations updated across all languages"))
            self.stdout.write("\nNow run: docker compose exec web python manage.py compilemessages")
        else:
            self.stdout.write(f"[DRY RUN] Would update {total_updated} translations total")
    ...
    def restore_formatting(self, original_text, translation_text):
        ...
        tag_pattern = r'<[^>]+>'
        tags = list(re.finditer(tag_pattern, original_text))
        ...
        for part in parts:
            if part.startswith('<') and part.endswith('>'):
                ...
                for i, text_part in enumerate(text_parts):
                    ...
                    new_text_parts.append(' '.join(new_words) if new_words else text_part.strip())
    ...
    def update_po_entries(self, po_entries, translations, english_lookup):
        ...
        for number, translation_data in translations.items():
            ...
            if number in english_lookup:
                ...
                formatted_translation = self.restore_formatting(original_msgid, clean_translation)
    ...
    def write_po_file(self, po_path, entries):
        ...
        header_end = header_match.start() + 1 if header_match else original_content.find('\nmsgid')
        if header_end == -1:
            header = ""
        else:
