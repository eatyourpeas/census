...
from .oidc_views import HealthcareOIDCAuthView, HealthcareOIDCCallbackView, oidc_logout_view
import logging
...
def oidc_success_debug_view(request):
    ...
    logger.info(f"OIDC Success: user={request.user}, authenticated={request.user.is_authenticated}")
