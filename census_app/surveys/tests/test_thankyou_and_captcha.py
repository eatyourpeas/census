...
def test_thank_you_route(client, settings, django_user_model):
    ...
    s = Survey.objects.create(owner=user, name="S", slug="s", status=Survey.Status.PUBLISHED, visibility=Survey.Visibility.PUBLIC)
...
def test_captcha_required_blocks_without_token(client, settings, django_user_model):
    ...
    s = Survey.objects.create(owner=user, name="S2", slug="s2", status=Survey.Status.PUBLISHED, visibility=Survey.Visibility.PUBLIC, captcha_required=True)
