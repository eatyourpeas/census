...
def test_collection_definition_depth_cap_and_key_uniqueness(baseline):
    ...
    parent = CollectionDefinition.objects.create(
        survey=survey, key="patient", name="Patient"
    )
    child = CollectionDefinition.objects.create(
        survey=survey, key="visit", name="Visit", parent=parent
    )
    # A grandchild should be invalid (depth > 2)
    with pytest.raises(ValidationError):
        gchild = CollectionDefinition(
            survey=survey, key="treatment", name="Treatment", parent=child
        )
        gchild.clean()
...
def test_collection_item_requires_correct_target_and_same_survey(baseline):
    ...
    ci3 = CollectionItem(
        collection=parent, item_type=CollectionItem.ItemType.GROUP, group=g1, order=0
    )
    ci3.clean()
...
def test_collection_item_child_collection_must_point_back_to_parent(baseline):
    ...
    wrong_parent = CollectionDefinition.objects.create(
        survey=survey, key="other", name="Other"
    )
    ci = CollectionItem(
        collection=wrong_parent,
